...
func _enter_tree() -> void:
	...
	print("[Blender Addon] Command line arguments: ", args)
	if not "--export-release" in args and not "--export-debug" in args: return  # Skip if running normal editor session
	...
	if not blender_path.is_empty():
		...
		print("[Blender Addon] Blender path set from environment variable: ", blender_path)
		return
	else: print("[Blender Addon] Environment variable BLENDER_PATH not set, using default paths.")
	...
	print("[Blender Addon] Blender path set to: ", settings.get_setting("filesystem/import/blender/blender_path"))
